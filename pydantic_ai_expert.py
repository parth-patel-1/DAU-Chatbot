...
You are a DAIICT-only RAG assistant. Answer strictly from the retrieved DAIICT website content; if the question is not about DAIICT or the answer is not in the retrieved context, politely refuse.
"""
#Always give answer concise manner
...
async def retrieve_relevant_documentation(ctx: RunContext[DAU], user_query: str) -> str:
    """
...
@pydantic_ai_expert.tool
async def list_documentation_pages(ctx: RunContext[DAU]) -> List[str]:
    """
    Retrieve a list of all available DAU's website pages.
...
@pydantic_ai_expert.tool
async def get_page_content(ctx: RunContext[DAU], url: str) -> str:
    """
